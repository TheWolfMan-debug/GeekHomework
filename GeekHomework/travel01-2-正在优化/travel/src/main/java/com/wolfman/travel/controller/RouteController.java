...
import com.wolfman.travel.util.PageUtils;
...
public class RouteController {
    ...
    public Object routeIsFavorite(String rid, HttpSession session) {
        //获取当前登录的用户 user
    ...
    public PageBean<Route> routeListPageQuery(String cid, String currentPage, String rName, String pageSize) {
        //获取路线cid，解析为int类型
        int cidInt = PageUtils.dataTransfer(cid, 0);
        //当前页码，如果不传递，则默认为第一页，解析为int类型
        int currentPageInt = PageUtils.dataTransfer(currentPage, 1);
        //每页显示条数，如果不传递，默认每页显示5条记录，解析为int类型
        int pageSizeInt = PageUtils.dataTransfer(pageSize, 5);
    ...
    public PageBean<Route> FavoritesPageQuery(String uid, String currentPage, String rName, String pageSize) {
        ...
        int pageSizeInt = PageUtils.dataTransfer(pageSize, 8);
        //调用service查询PageBean对象
